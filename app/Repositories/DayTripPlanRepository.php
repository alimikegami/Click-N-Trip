...
class DayTripPlanRepository
{
    ...
    {
        $res = DB::select('SELECT r.*, u.email FROM reservation r INNER JOIN day_trip_plan dtp ON r.day_trip_plan_id = dtp.id INNER JOIN users u ON r.user_id = u.id WHERE dtp.user_id = ? AND r.day_trip_plan_id = ?', [$userId, $dtpId]);
    ...
    {
        $res = DB::select("SELECT dtp.*, COUNT(r.day_trip_plan_id) AS res_count,ROUND(AVG(r2.star_count)) AS star_count, dti.image_path FROM day_trip_plan dtp LEFT JOIN reservation r ON dtp.id = r.day_trip_plan_id LEFT JOIN review r2 ON r2.reservation_id = r.id LEFT JOIN day_trip_image dti ON dti.day_trip_plan_id = dtp.id GROUP BY r.day_trip_plan_id ORDER BY res_count DESC LIMIT 3");
        return $res;
    }
}
